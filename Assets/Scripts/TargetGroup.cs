...
/// <summary>
/// Adaption of <see cref="CinemachineTargetGroup"/> with <see cref="Vector3"/> weights, which allows different weights per dimension.<br/>
/// <p>Defines a group of target objects, each with a radius and a weight.
...
/// and radii into account.</p>
/// </summary>
[AddComponentMenu("Cinemachine/TargetGroup")]
[SaveDuringPlay]
[ExecuteAlways]
[DisallowMultipleComponent]
//[HelpURL(Documentation.BaseURL + "manual/CinemachineTargetGroup.html")]
public class TargetGroup : MonoBehaviour, ICinemachineTargetGroup
{
    ...
    [Serializable] public class Target
    {
        ...
        public Vector3 Weight = Vector3.one; // Changed to component-wise weights
    ...
    void OnValidate()
    {
        ...
        {
            Targets[i].Weight = MathV.Max(Vector3.zero, Targets[i].Weight);
    ...
    public void AddMember(Transform t, Vector3 weight, float radius)
    {
    ...
    static BoundingSphere WeightedMemberBoundsForValidMember(Target t, Vector3 avgPos, Vector3 maxWeight)
    {
        var pos = t.Object == null ? avgPos : TargetPositionCache.GetTargetPosition(t.Object);
        // Changed to component-wise equivalent
        var w = MathV.Max(Vector3.zero, t.Weight);
        for (int i = 0; i < 3; ++i)
        {
            if (maxWeight[i] > UnityVectorExtensions.Epsilon && w[i] < maxWeight[i])
                w[i] /= maxWeight[i];
            else
                w[i] = 1;

            pos[i] = Mathf.Lerp(avgPos[i], pos[i], w[i]);
        }
        return new BoundingSphere(pos, t.Radius * MathV.Sum(w) / 3f); // changed to average weight of the three components
    }
    ...
    void UpdateMemberValidity()
    {
        ...
        m_WeightSum = m_MaxWeight = Vector3.zero;
        for (int i = 0; i < count; ++i)
        {
            m_MemberValidity.Add(Targets[i].Object != null
                    && MathV.Sum(Targets[i].Weight) > UnityVectorExtensions.Epsilon
                    && Targets[i].Object.gameObject.activeInHierarchy);
            if (m_MemberValidity[i])
            {
                m_ValidMembers.Add(i);
                m_MaxWeight = MathV.Max(m_MaxWeight, Targets[i].Weight);
    ...
    {
        if (MathV.Sum(m_WeightSum) < UnityVectorExtensions.Epsilon)
        ...
        for (int i = 0; i < count; ++i)
        {
            ...
            pos += TargetPositionCache.GetTargetPosition(Targets[targetIndex].Object).Mul(weight);
        }
        return pos.Div(m_WeightSum);
    }

    // Assumes that UpdateMemberValidity() has been called
    Bounds CalculateBoundingBox()
    {
        if (MathV.Sum(m_MaxWeight) < UnityVectorExtensions.Epsilon)
            return m_BoundingBox;
    ...
    {
        var count = m_ValidMembers.Count;
        if (count == 0 || MathV.Sum(m_MaxWeight) < UnityVectorExtensions.Epsilon)
            return m_BoundingSphere;
    ...
    {
        if (MathV.Sum(m_WeightSum) > 0.001f)
        {
            ...
            {
                var targetIndex = m_ValidMembers[i];
                var scaledWeight = MathV.Sum(Targets[targetIndex].Weight) / MathV.Sum(m_WeightSum);
